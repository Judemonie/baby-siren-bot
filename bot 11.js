...
function isSocialsRequest(text) {
  ...
  return t === "socials" || t === "/socials" || t === "links" || t === "/links" ||
    t === "x" || t === "/x" || t === "twitter" || t === "/twitter" ||
    t.includes("social") || t.includes("where to follow") || t.includes("all links") ||
    t.includes("twitter") || t.includes("telegram link");
}
...
bot.on("text", async function(ctx) {
  try {
    ...
    if (text.startsWith("/") && !isCARequest(text) && !isSocialsRequest(text)) return;
    ...
    // CA request — image with varied HTML caption + CopyTextButton
